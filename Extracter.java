...
public class Extracter {
    ...
    public static List<String> findDropper(Element table){
        Elements dropNames = table.select(".ak-main-content .ak-title .ak-linker");
        Elements dropRates = table.select(".ak-main-content .ak-aside");
        List<String> dropList = new ArrayList<>(dropNames.size());
        for(int i =0;i<dropNames.size();i++){
            Element ele1 = dropNames.get(i);
            Element ele2 = dropRates.get(i);
            if(ele1.text().length()>0){
                dropList.add("*[["+ele1.text()+"]] "+"''("+ele2.text()+")''");
            }
        }
        return dropList;
    }
